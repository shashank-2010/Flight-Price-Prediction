...
#first convert the duration into total minutes
...
sns.scatterplot(x = 'Total_duration(min)', y = 'Price', data = data_1, hue = 'Total_Stops')
plt.legend();
...
#sort the order of route where Jet airways is extremely used? - there are two rows
a = data_1[data_1['Airline'] == 'Jet Airways']
a.groupby('Route').size().sort_values(ascending = False)
...
def plot(df,col):
    ...
    sns.barplot(df[col], ax=ax2)
    sns.distplot(df[col],ax = ax3, kde = False) # for histogram
...
plot(data_copy,"Price")  #see the difference in the plots
...
imp_df = imp_df.rename(columns={0: 'Mutual_Regression'})  
imp_df
...
#more prediction based on X_train using the saved model - ml_model
